...
class APDrawUtilities: NSObject {
    static func rectBetween(_ firstPoint: CGPoint, _ secondPoint: CGPoint) -> CGRect {
        let points = APDrawUtilities.beginAndEndPointForRectBetweenPoints(firstPoint, secondPoint)
        return CGRect(x: points.0.x, y: points.0.y, width: points.1.x - points.0.x, height: points.1.y - points.0.y)
    }

    private static func beginAndEndPointForRectBetweenPoints(_ firstPoint: CGPoint,
                                                             _ secondPoint: CGPoint) -> (CGPoint, CGPoint) {
    ...
    static func convert(_ point: CGPoint, fromViewWithSize viewSize: CGSize,
                        andContentInAspectFitModeWithSize contentSize: CGSize) -> CGPoint {
