...
class APAuthManager: NSObject, MSAuthenticationProvider {
    ...
    public func getTokenInteractively(parentViewController: UIViewController, completion: @escaping(_ accessToken: String?, Error?) -> Void) {
        let webParameters = MSALWebviewParameters.init(authPresentationViewController: parentViewController)
